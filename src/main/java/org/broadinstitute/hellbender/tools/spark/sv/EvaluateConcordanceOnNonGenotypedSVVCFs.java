...
public final class EvaluateConcordanceOnNonGenotypedSVVCFs extends AbstractIntervalTreeBasedConcordanceWalker {
    ...
    /**
     * For testing if two variant context's spanning intervals overlap,
     * potentially allowing padding around variants via overrides.
     *
     * Potential overrides, e.g. SV concordance walkers, may like to allow more padding,
     * and even consider reference context for record by record customization.
     */
    @Argument(doc = "Padding around variants for overlapping", fullName = "padding", optional = true)
    protected Integer padding = 50;

    @Argument(doc = "prefix for output", shortName = StandardArgumentDefinitions.OUTPUT_SHORT_NAME, fullName = StandardArgumentDefinitions.OUTPUT_LONG_NAME)
    private String outPrefix;

    @Argument(doc = "Minimum reciprocal overlap fraction for two concordant variants of the type DEL/INV",
            fullName = "recip-ovp-frac", optional = true)
    private Float recipOvpFrac = 0.5f;

    @Argument(doc = "Maximum difference in length between two concordant INS/DUP variants, " +
    ...
    @Argument(doc = "Minimum mapping quality of the evidence contig mappings; setting to zero turns off this filter",
    ...
    private Float coefficient; // TODO: 6/4/18 this coefficient is initialized in a way that $ 1 - coeffient * lenDiff/Math.min(evalLen, truthLen) < recipOvpFrac $ for discordant variants
    ...
    protected Predicate<VariantContext> makeEvalVariantFilter() {
        return vc -> {
            ...
            if (vc.hasAttribute(GATKSVVCFConstants.MAX_ALIGN_LENGTH)) {
                if (vc.getAttributeAsInt(GATKSVVCFConstants.MAX_ALIGN_LENGTH, 0) < minAlnLen) return false;
            }
            return true;
        };
    }
    ...
    protected void apply(final VariantContext eval, final ReadsContext readsContext, final ReferenceContext refContext) {
        ...
        if (truthTree.hasOverlapper(evalInterval)) {
            ...
            if (supportingTruth == null) {
                ...
                // TODO: 6/3/18 can compactify into a single record
                if (eval.isFiltered()){
                    classifiedVariants.add(new FilteredFalseNegative(eval, supportingTruth));
                } else {
                    classifiedVariants.add(new TruePositive(eval, supportingTruth));
                }
            }
        } else {
            if (eval.isFiltered()) {
    ...
    protected SVInterval getPaddedSvInterval(final VariantContext eval) {
        return new SVInterval(refSeqDict.getSequenceIndex(eval.getContig()),
                    ...
                    Math.min(eval.getEnd() + padding, refSeqDict.getSequence(eval.getContig()).getSequenceLength()));
    }
    ...
    private SupportingTruth getConcordantTruthCalls(final VariantContext eval, final SVInterval evalInterval) {
        ...
        final Iterator<SVIntervalTree.Entry<List<TruthVariant>>> overlappingTruths =
                truthTree.overlappers(evalInterval);
...
//                        (classifiedVariant instanceof TruePositive || classifiedVariant instanceof FilteredFalseNegative)
    ...
    private static <T extends Enum<T>> EnumMap<T, Long> getZeroInitializedEnumMap(Class<T> clazz) {
